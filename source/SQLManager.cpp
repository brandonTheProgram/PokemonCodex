...
void SQLManager::executeQuery(const std::string& query) {
    ...
    try {
        ...
        this->logger.critical("SQLManager::executeQuery Failed to execute query: " + query + ". Error message: " + std::string(e.what()));
    }
}
...
void SQLManager::prepareStatement(const std::string& query) {
    ...
    try {
        ...
        this->logger.critical("SQLManager::prepareStatement Failed to prepare the statement for the query: " + query + ". Error message: " + std::string(e.what()));
    }
}
...
void SQLManager::bind(const int& index, const int& value) {
    ...
    try {
        ...
        this->logger.critical("SQLManager::bind Failed to bind: " + std::to_string(value) + " to index: " + std::to_string(index) + ". Error message: " + std::string(e.what()));
    }
}
...
void SQLManager::bind(const int& index, const std::string& value) {
    ...
    try {
        ...
        this->logger.critical("SQLManager::bind Failed to bind: " + value + " to index: " + std::to_string(index) + ". Error message: " + std::string(e.what()));
    }
}
    ...
    try {
        while (this->stmt_->executeStep()) {
            ...
            for (int i = 0; i < this->stmt_->getColumnCount(); i++) {
                row.push_back(this->stmt_->getColumn(i).isNull() ? "NULL" : this->stmt_->getColumn(i).getText());
            }
